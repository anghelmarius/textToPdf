...
if __name__ == '__main__':
    alltext = open_file('summary.txt')
    chunks = textwrap.wrap(alltext, 2000)
    result = list()
    count = 0
    for chunk in chunks:
        count = count + 1
        augmented_prompt = f"as long as the answer is contained by this data: '{alltext}', reply only using the information in the data."
        prompt = augmented_prompt.encode(encoding='ASCII',errors='ignore').decode()
